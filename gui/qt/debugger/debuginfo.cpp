    ...
    if (val.is_loclist()) {
        ...
        for (auto loc : val.loclist()) {
            if (first)
                ...
                debug << ", ";
            debug << '[' << loc._M_range.first << ',' << loc._M_range.second << ") " << loc._M_desc;
        }
        return debug;
    }
    if (val.is_rnglist()) {
...
auto Info::_Scope::child(_Addr addr) const noexcept -> const _Scope & {
...

    {
        ...
        for (const _Die &entry : _M_entries) {
            ...
            if (low_pc.is_addr() && high_pc.is_addr())
                stack.back().second.get().child(low_pc.addr(), high_pc.addr(), _M_scopes.back());
            else if (ranges.is_rnglist())
                for (const auto &rng : ranges.rnglist())
                    stack.back().second.get().child(rng.first, rng.second, _M_scopes.back());
